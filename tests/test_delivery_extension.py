...
class TestDeliveryExtension:
    ...
    def test_extend_subscription_without_form(self, client, sample_data):
        """Тест продовження підписки без форми (прямий API)"""
        unpaid_delivery_id = sample_data['unpaid_delivery_id']
        
        response = client.post(f'/deliveries/{unpaid_delivery_id}/extend-subscription')
    ...
    def test_extend_subscription_with_form_valid_data(self, client, sample_data):
        ...
        response = client.post(
            f'/deliveries/{unpaid_delivery.id}/extend-subscription-with-form',
            data=form_data
        )
    ...
    def test_extend_subscription_missing_required_fields(self, client, sample_data):
        ...
        unpaid_delivery = sample_data['unpaid_delivery']
    ...
    def test_extend_subscription_invalid_delivery(self, client, sample_data):
        """Тест продовження підписки для неправильної доставки"""
        # Спробуємо продовжити доставку, яка не є неоплаченою
        paid_delivery = Delivery.query.filter_by(
            status='Очікує',
            is_subscription=True
        ).first()
        ...
        response = client.post(
            f'/deliveries/{paid_delivery.id}/extend-subscription-with-form',
            data=form_data
        )
    ...
    def test_extend_subscription_nonexistent_client(self, client, sample_data):
        """Тест продовження підписки з неіснуючим клієнтом"""
        unpaid_delivery = sample_data['unpaid_delivery']
        ...
        data = json.loads(response.data)
        assert data['success'] == False
        assert 'не знайдений' in data['error'].lower()
    ...
    def test_extend_subscription_custom_size_with_amount(self, client, sample_data):
        ...
        unpaid_delivery = sample_data['unpaid_delivery']
        test_client = sample_data['client']
        
        form_data = {
            'client_instagram': test_client.instagram,
            'recipient_name': 'Test',
            'recipient_phone': '+380123456789',
            'city': 'Київ',
            'delivery_type': 'Weekly',
