...
- `POST /api/integrations/wix/order-placed` — публічний вебхук.
  - Авторизація: Wix Automations не дозволяє додавати кастомні HTTP-заголовки
    до вихідного запиту, тож токен у заголовку не підходить. Замість цього
    звіряємо поле `data.context.metaSiteId` із тіла запиту — стабільний ID
    Wix-сайту, який проставляє сам Wix. Дозволені ID — список в env
    `WIX_ALLOWED_SITE_IDS` (comma-separated, напр.
    `2c31eac1-a4f1-4fd2-886d-361377a42a1f`). `metaSiteId` відсутній у списку
    (або відсутній у payload) → `403`.
    **Важливе уточнення (виявлено при фінальному рев'ю):** `metaSiteId` — це
    НЕ секрет. Він публічно присутній у вихідному коді/viewer-моделі кожного
    Wix-сайту, тож будь-який відвідувач сайту технічно може його прочитати.
    Захист тут — не "зловмисник не знає значення", а те, що URL вебхука
    (`/api/integrations/wix/order-placed`) відомий лише тому, хто налаштовує
    Wix Automation, і не публікується на сайті. Реальний ризик при витоку
    `metaSiteId` — спам фейкових `WixLead`-записів у списку заявок
    (обмежено `MAX_CONTENT_LENGTH` на запит), не витік даних. Для захисту в
    глибину можна додатково додати секретний сегмент у сам шлях вебхука
    (`/api/integrations/wix/order-placed/<секрет>`), оскільки Wix повністю
    контролює URL, на який шле POST, навіть без можливості кастомних
    заголовків — це залишено як рекомендація на майбутнє, не блокер для MVP.
  - Тіло розбирається як JSON (`request.get_json(silent=True)`); порожнє/невалідне
    тіло → `400`, лід не створюється.
  - На дозволений `metaSiteId` + валідний JSON — `200 {"ok": true, "lead_id": ...}`,
    навіть якщо парсинг частковий (сирий payload все одно зберігається,
    помилки логуються) — щоб Wix не ретраїв і жодні дані не губились. Виняток:
    якщо в payload відсутній `data.id` (унікальний Wix order id, ключ
    ідемпотентності) — `400`, без ліда, оскільки без нього неможливо уникнути
    дублів при ретраях; це свідомий відхід від "завжди 200" (виявлено при
    фінальному рев'ю), обґрунтований тим, що лід без ідентифікатора все одно
    марний.
...
- `WIX_ALLOWED_SITE_IDS` не заданий (порожній) в env → ендпоінт відповідає `503`
  (інтеграція не сконфігурована), щоб не тримати публічний ендпоінт відкритим
  за замовчуванням і випадково не приймати запити з будь-яким `metaSiteId`.
...
- Webhook auth: запит з `metaSiteId`, якого немає в `WIX_ALLOWED_SITE_IDS` (або
  взагалі без цього поля) → `403`; без `WIX_ALLOWED_SITE_IDS` в конфігу → `503`.
