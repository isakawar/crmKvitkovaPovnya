...
- `POST /api/integrations/wix/order-placed` — публічний вебхук.
  - Авторизація: Wix Automations не дозволяє додавати кастомні HTTP-заголовки
    до вихідного запиту, тож токен у заголовку/query не підходить (Postman міг
    би підставити той самий query-параметр, підглянувши URL). Замість цього
    звіряємо поле `data.context.metaSiteId` із тіла запиту — унікальний,
    стабільний ID Wix-сайту, який проставляє сам Wix і якого зловмисник не
    знає. Дозволені ID — список в env `WIX_ALLOWED_SITE_IDS` (comma-separated,
    напр. `2c31eac1-a4f1-4fd2-886d-361377a42a1f`). `metaSiteId` відсутній у
    списку (або відсутній у payload) → `403`.
